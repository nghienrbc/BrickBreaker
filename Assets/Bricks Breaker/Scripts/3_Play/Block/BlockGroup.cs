...
public class BlockGroup : MonoBehaviour
{
    ...
    public void SetBlockGroup(int num = 0, int health = 0)
    {
        ...
        bool isClassicGame = CtrGame.instance.isClassicGame;

        for (int i = 0; i < CtrBlock.instance.firstTransform.Count; i++)
        {
            //Addball
            int itemID = Random.Range(0, 5);
            bool blockHasItem = CheckItemSpawn();
            if (bCount > i)
            {
                if (i == 0)
                {
                ...
                //Block data setting
                blockBase.SetData(health, !isClassicGame && blockHasItem ? itemID : 0);
            }
        }
    }
    ...
    bool CheckItemSpawn()
    {
        ...
        else if (levelNumber > 20 && levelNumber <= 25)
        {
            per1 = 50;
            per2 = 100;
            if (n >= per1) result = true;
    ...
    int GetBlockCount()
    {
        int turnCount = CtrGame.instance.isClassicGame ? CtrGame.instance.turnCount : PlayerPrefs.GetInt("levelNumber") * 2 +  1;
        int n = Random.Range(0, 100);
        int blockcount = 0;
