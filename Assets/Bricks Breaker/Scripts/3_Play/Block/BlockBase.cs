...
public class BlockBase : MonoBehaviour {
    ...
    public virtual void SetData (int health, int itemId = 0) {
        itemID = itemId;
        isDestory = false;
        if (textHealht != null) {
    ...
    public virtual void InDamage (Collision2D collision) {
        ...
        if (blockHealth <= 0) {
            ...
            // nếu block bị phả hủy thì kiểm tra itemid có khác 0 không để spawn item
            if(itemID != 0)
            {
                Item newItem = PoolManager.Spawn(CtrPool.instance.pItem, transform.position, Quaternion.identity).GetComponent<Item>();
                newItem.SetData(itemID); 
            } 
        }
    }
