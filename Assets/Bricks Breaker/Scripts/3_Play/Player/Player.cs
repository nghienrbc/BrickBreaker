...
public class Player : MonoBehaviour
{
    ...
    IEnumerator ShotBallCo()
    {
        ...
        for (int i = 0; i < ballMaxCount; i++)
        {
            ...
            activeBall.Add(ball);
            if (CtrGame.instance.isClassicGame) ball.SetData(1); //damage = 1
            else ball.SetDataInArcadeMode(levelNumber > 3 ? (int)levelNumber/3 : 1); //damage = levelNumber
    ...
    IEnumerator CheckTurnCo()
    {
        ...
        // nếu là game arcade thì tới đây là gameover
        if (!CtrGame.instance.isClassicGame && !CtrGame.instance.isCompleteLevel)
        {
    ...
    IEnumerator ReturnBallCo()
    {
        ...
        if (CtrGame.instance.isClassicGame) StartCoroutine(ReadyPlayerCo());
    }
    #endregion
}
