...
public class PanelStart : MonoBehaviour
{
    ...
    public void SetPanelStart(string gameType)
    {
    ...
    public void Click_Replay(string gametype)
    {
        if (isClick) return;
        isClick = true;
    ...
    void RePlay(string gametype)
    {
        PlayManager.Instance.isSaveGameStart = true;
        Click_Play(gametype);
    }

    public void Click_Play(string gametype)
    {
        bool isClassicGame = true;// CtrGame.instance.isClassicGame;
        PlayManager.Instance.LoadScene(gametype == Data.game_type_classic ? Data.scene_play : Data.scene_play_level);
